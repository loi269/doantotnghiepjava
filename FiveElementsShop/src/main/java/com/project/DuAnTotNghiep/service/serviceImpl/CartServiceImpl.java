...
import com.project.DuAnTotNghiep.repository.CartRepository;
...
import com.project.DuAnTotNghiep.repository.PaymentMethodRepository;
...
public class CartServiceImpl implements CartService {
    ...
    public void orderUser(OrderDto orderDto) {
        ...
        String subject = "Xác nhận đơn hàng " + billNew.getCode();
        String htmlContent = buildOrderConfirmationEmail(billNew, billDetailList);
        try {
            emailService.sendEmail(UserLoginUtil.getCurrentLogin().getEmail(), subject, htmlContent);
    ...
    public String buildOrderConfirmationEmail(Bill bill, List<BillDetail> billDetailList) {
        StringBuilder content = new StringBuilder();

        content.append("<h1>Đơn hàng của bạn</h1>")
                .append("<p>Cảm ơn bạn đã mua hàng! Dưới đây là chi tiết đơn hàng của bạn::</p>")
                .append("<p><strong>Mã đơn hàng:</strong> ").append(bill.getCode()).append("</p>")
                .append("<p><strong>Tổng tiền:</strong> ").append(bill.getAmount()).append("</p>")
                .append("<h2>Thông tin sản phẩm</h2>")
        ...
        for (BillDetail detail : billDetailList) {
            content.append("<tr>")
                    ...
                    .append("<td>").append(detail.getProductDetail().getSize().getName()).append("</td>")
                    .append("<td>").append(detail.getProductDetail().getColor().getName()).append("</td>")
                    .append("<td>").append(detail.getQuantity()).append("</td>")
                    .append("<td>").append(detail.getMomentPrice()).append("</td>")
                    .append("<td>").append(detail.getQuantity() * detail.getMomentPrice()).append("</td>")
                    .append("</tr>");
        }
    ...
    public OrderDto orderAdmin(OrderDto orderDto) {
        ...
        return new OrderDto(billNew.getId().toString(), orderDto.getCustomer(), billNew.getInvoiceType(),
                billNew.getStatus(), billNew.getPaymentMethod().getId(), billNew.getBillingAddress(),
                billNew.getPromotionPrice(), null, null, null);
    }
